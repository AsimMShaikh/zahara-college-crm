...
const reasons = [
  ...
  ["A future within reach", "Accessible programmes that keep career-building affordable."],
  ["Support that feels personal", "Guidance to help you choose a path that fits your interests."]
];
...
export function HomePage({ careerPaths, courses, heroCourses, faqs, stories }: HomePageProps) {
  ...
  return <main>
    ...
    <section id="about" className="bg-mist px-5 py-20 sm:py-24 lg:px-8"><div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[.82fr_1.18fr] lg:items-start"><SectionHeading align="left" eyebrow="Why Zahara" title="A college that sees the person behind every aspiration." description="Zahara exists to provide direction and a platform for young women seeking opportunities that fit their interests and the world of work." /><div className="grid gap-4 sm:grid-cols-2">{reasons.map(([title, description], i) => <div key={title} className="rounded-3xl bg-white p-6 shadow-card"><span className="grid size-10 place-items-center rounded-2xl bg-brand-50 font-display text-base font-bold text-brand-700">0{i + 1}</span><h3 className="mt-5 font-display text-xl font-bold text-ink">{title}</h3><p className="mt-3 text-sm leading-6 text-slate-600">{description}</p></div>)}</div></div></section>

    <CoursesSection allCourses={courses} />
    ...
    <section id="admissions" className="px-5 py-20 sm:py-24 lg:px-8"><div className="mx-auto max-w-7xl"><SectionHeading eyebrow="Admissions" title="Questions are a good place to start." description="We’ll make the process simple and help you understand your options." /><FAQList faqs={faqs} /></div></section>

    <section className="px-5 pb-20 lg:px-8"><div className="mx-auto max-w-7xl overflow-hidden rounded-4xl bg-brand-600 px-7 py-12 text-white sm:px-12 sm:py-16"><div className="grid gap-8 lg:grid-cols-[1.15fr_.85fr] lg:items-end"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-brand-100">Your future begins here</p><h2 className="mt-4 max-w-xl font-display text-4xl font-bold leading-tight sm:text-5xl">Every journey starts with one step.</h2><p className="mt-5 max-w-xl text-lg leading-8 text-brand-100">Take the first step toward practical skills, personal confidence and new possibilities.</p></div><div className="flex flex-col gap-3 sm:flex-row lg:justify-end"><Button variant="light" size="lg" asChild><Link href="/admissions">Start your journey <ArrowRight className="size-4" /></Link></Button><Button variant="outline" size="lg" className="border-white/35 bg-transparent text-white hover:bg-white/10 hover:text-white" asChild><a href={whatsAppUrl} target="_blank" rel="noreferrer"><MessageCircle className="size-4" /> Chat on WhatsApp</a></Button></div></div></div></section>
  </main>;
}
